...
class newsController {
    ...
    add_rating = async (req, res) => {
        try {
            ...
            news.averageRating = parseFloat(avg.toFixed(1)); // Store as a number with 1 decimal
    ...
    add_comment = async (req, res) => {
        const { news_id } = req.params;
        const { name, comment } = req.body;  // Accept name and comment from the body
        ...
        try {
            ...
            // Create the comment object without userId
            const newComment = {
                name,    // Just store the name
                text: comment, // Store the actual comment text
            };
            ...
            console.error('Error during comment addition:', error);  // Log detailed error
            return res.status(500).json({ message: 'Internal server error' });
        }
    };
